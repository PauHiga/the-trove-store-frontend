...
const NavBar = () => {
  ...
  const amountItemsInCart = currentCart.reduce(
    ...
    <StyledNavBar>
      <div>
        <ul>
          <li>
            {user ? (
              <Link to="/user-section" id="user-section">
                <img src={iconLogin} alt="icon search" />
                <span className="navbar-label">{user.username} logged in</span>
              </Link>
            ) : (
          ...
          <li>
            <Badge count={amountItemsInCart} color="#ce9124" offset={[-10, 0]}>
              <Link to="/cart" id="cart">
                <img src={iconCart} alt="icon search" />
                <span className="navbar-label">Cart</span>
              </Link>
            </Badge>
          </li>
        </ul>
