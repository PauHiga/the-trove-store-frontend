...
const EditProducts = () => {
  ...
  const productCategories =
    currentProduct.category.length > 0
      ? currentProduct.category.map((item) => item.id)
      : [];
  ...
  const handleSubmit = async (e) => {
    ...
    const stockLargerThanZero = keysArray.reduce(
      (sum, item) => sum + stock[item],
      0,
    );
  ...
  const handleCancel = () => {
    const confirm = window.confirm(
      "Are you sure you want to cancel? No changes will be submitted",
    );
    if (confirm) {
      navigate("/user-section");
    }
    ...
    <div>
      ...
      <EditProductsForm onSubmit={handleSubmit}>
        ...
        <div className="stock">
          <div
            className={
              !productWithSize ? "stock-option" : "stock-option-disabled"
            }
            onClick={allowProductWithUniqueSize}
          >
            <div className="form-entry">
