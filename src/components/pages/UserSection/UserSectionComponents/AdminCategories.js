...
const AdminCategories = () => {
  ...
  const handleAddCategory = async () => {
    if (newCategory.length === 0) {
      toast(
        "The category name cannot be empty. Please add a name for the new category.",
      );
      setErrorMessage(
        "The category name cannot be empty. Add a name for the new category.",
      );
      ...
      <StyledAdminCategories>
        <div>
          <div className="inline">
            <h2>Categories</h2>
            <div
              className="toggle-add-category"
              data-bs-toggle="collapse"
              data-bs-target="#collapse-add-category"
              aria-controls="collapse-add-category"
            >
          ...
          <div>
            ...
            <ul>
              {categories.map((item) => (
                <EditCategories item={item} key={item.id} />
              ))}
            </ul>
          </div>
        </div>
